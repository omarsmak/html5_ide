...
class TreeComponent extends Component {
    ...
    public function __construct($table = "tree", $fields = array()){
        //App::import('model','TreeFile');
        //$this->TreeFileModel = new TreeFile();
        //$results = $this->Databasehelper->query("select * from ide_files");
        //print_r($results);
        ...
        //$this->db->query("select * from ide_files where file_id = 2");
        //$this->db->nextr();
        ...
        $this->file = new FilesoperationsComponent() ; 
    ...
    function _get_parent($id,$recursive = false){
        $parent = array();
        
        //read the same id
        $node = $this->_get_node($id);
        $parent[$id] = $node;
        //start reading the parents ID's'
        $this->db->query("SELECT `".implode("` , `", $this->fields)."` FROM `".$this->table."` WHERE `".$this->fields["id"]."` = ".(int) $node[$this->fields["parent_id"]]);
        
        while($this->db->nextr()){
            $prev_id = $this->db->f($this->fields["id"]);
            $parent[$prev_id] = $this->db->get_row("assoc");
            $this->db->query("SELECT `".implode("` , `", $this->fields)."` FROM `".$this->table."` WHERE `".$this->fields["id"]."` = ".(int) $parent[$prev_id]["parent_id"]);
        }
        return $parent;
