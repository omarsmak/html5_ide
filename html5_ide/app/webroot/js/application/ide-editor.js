...
function IdeEditor(place, options, mirrorOptions) {
...
IdeEditor.prototype = {
  ...
  initialize: function(textarea, options, mirrorOptions) {
    var defaultOptions = {
      ...
      path: 'js',
      buttons: ['save','search','code_bits', 'undo', 'redo', 'jump', 'reindentSelection', 'reindent','preview']
    }
    ...
    this.buttonDefs = {
      'save' :  ["Save", "save", this.options.path + "../images/editor_icons/document-save-as2.png", this.save] ,
      'search': ["Search/Replace", "find_replace_popup", this.options.path + "../images/editor_icons/edit-find-replace2.png", this.find_replace_popup],
      ...
      'undo': ["Undo", "undo", this.options.path + "../images/editor_icons/edit-undo2.png", this.undo],
      'redo': ["Redo", "redo", this.options.path + "../images/editor_icons/edit-redo2.png", this.redo],
      'jump': ["Jump to line #", "jump", this.options.path + "../images/editor_icons/go-jump2.png", this.jump],
      'reindentSelection': ["Reformat selection", "reindentSelect", this.options.path + "../images/editor_icons/format-indent-more2.png", this.reindentSelection],
      'reindent': ["Reformat whole document", "reindent", this.options.path + "../images/editor_icons/format-justify-left2.png", this.reindent],
      'about': ["About CodeMirror-UI", "about", this.options.path + "../images/editor_icons/dialog-information2.png", this.about],
      'code_bits' : ["CodeBits", "codebits", this.options.path + "../images/editor_icons/preferences-system2.png", this.codebits], 
      'preview' : ["Preview", "preview", this.options.path + "../images/editor_icons/text-html2.png", this.preview]
    };

    //place = CodeMirror.replace(place)

    this.home = document.createElement("div");
    this.home.className = "test";
  ...
  changeName : function(){
    var file_id = this.textarea.id.split("code_")[1];
   ...
   */
    ...
    preview : function(){
        file_id = this.textarea.id.split("code_")[1];
        _app.utility.preview_file(file_id);
    },
  ...
  codebits : function(){
    ...
    //test
    var elem = document.getElementById("code_bits_menu");
    var mirror = this.mirror;
    if(elem){
        //this elem exist
        //remove it 
        this.home.removeChild(elem);
    }else {
        var codebits_menu = document.createElement("div");
        codebits_menu.id = "code_bits_menu";
        
        
        //alert($("#code_bits_menu").html());
        //codebits_menu.innerHTML = '<ul><li><ins>&nbsp;</ins><a href="#"><span>&raquo;</span>Sub Item 1.1</a><ul><li><a href="#">ddd</a></li><li><a href="#">ddd</a></li></ul></li><li><a href="#">Sub Item 1.2</a><ul><li><a href="#">DDDDDD</a></li></ul></li><li><a href="#">Sub Item 1.2</a><ul><li><a href="#">ddd</a></li><li><a href="#">sssss</a></li></ul></li></ul>';
        this.home.appendChild(codebits_menu);
        
        _app.codebits.load_all_code_bits(function(output){
            //alert(output);
            //codebits_menu.innerHTML = output;
                $("#code_bits_menu").html(output);
            
                $("#code_bits_menu").find("li:has(ul)").bind("mouseover",function(e){
                ...
                $("#code_bits_menu").bind("mouseleave",function(e){
                    $(this).remove();
                });
            
                $("#code_bits_menu").find("li:not(:has(ul))").click(function(){
                //alert($(this).html());
                //check if smth selected 

                code_id = $(this).attr("id");
                _app.codebits.load_code_bit(function(output){
                    
                    if(mirror.somethingSelected()){
                        mirror.replaceSelection(output);
                    } else {
                        //put in the current currsonr
                        mirror.replaceRange(output,mirror.getCursor());
                    }
  ...
  save : function(){
    data = this.get_value();
    editor_changed = this.editor_changed;
    //alert(data);
    var file_id = this.textarea.id.split("code_")[1];
    _app.utility.save_file(function(output){
        //alert(output.status);
        if(output.status){
            //alert(editor_changed);
            if(editor_changed){
                $("#tabs ul li a").each(function(index){
                    if($(this).attr("rel") == file_id){
                            filename = $(this).find("span").html().split("*")[0];
                            //alert(filename);
                            $(this).find("span").html(filename);
                            _this.editor_changed = false ;
                        }
                });
            }
        }
